...
//
// TURN (для стабильного WebRTC между разными сетями / NAT):
//   set TURN_URLS=turn:YOUR_VPS_IP:3478?transport=udp,turn:YOUR_VPS_IP:3478?transport=tcp
//   set TURN_USERNAME=turnuser
//   set TURN_PASSWORD=turnpass
//
// PERF (ускорить регистрацию на слабом CPU):
//   set BCRYPT_ROUNDS=10   (8..14)
//
// open:
...
// ---------------- PERF / SECURITY knobs ----------------
// bcrypt rounds affect signup/login speed on weak CPUs (Render free can be slow).
// recommended: 10 for MVP, 12 for stronger.
const BCRYPT_ROUNDS = Math.max(8, Math.min(14, Number(process.env.BCRYPT_ROUNDS || 10)));

// ---------------- TURN (optional, required for stable WebRTC across networks) ----------------
// Render cannot be TURN. Use external VPS (coturn) and set these env vars on Render service.
...
async function ensureSuperadmin() {
  ...
  if (!exists) {
    const passwordHash = await bcrypt.hash(SP, BCRYPT_ROUNDS);
    data.users.push({
      username: SU,
      passwordHash,
      role: "superadmin",
...
app.patch("/api/admin/users/:username", authMiddleware, requireRole("dept_admin"), async (req, res) => {
  ...
  if (typeof req.body.newPassword === "string" && req.body.newPassword.length) {
    ...
    u.passwordHash = await bcrypt.hash(np, BCRYPT_ROUNDS);
  }
...
function computeSummaryForRole(reqUser) {
  ...
  for (const r of allRooms) {
    ...
    if (r.qos && typeof r.qos.avgRtt === "number") {
      rttSum += r.qos.avgRtt;
      rttN++;
    }
    if (r.qos && typeof r.qos.avgLoss === "number") {
      lossSum += r.qos.avgLoss;
      lossN++;
    }

    if (r.count >= 2) {
      const durSec = r.startedAt
        ? Math.max(0, Math.floor((Date.parse(new Date().toISOString()) - Date.parse(r.startedAt)) / 1000))
        : 0;
      liveList.push({
        roomId: r.roomId,
        type: r.type,
  ...
  let peakHour = 0,
    peakValue = -1;
  for (let i = 0; i < 24; i++) {
    if (hourBuckets[i] > peakValue) {
      peakValue = hourBuckets[i];
      peakHour = i;
    }
  }
...
function handleAdminAction(user, msg) {
  ...
  if (action === "end-room") {
    ...
    for (const [, m] of targetRoom.members.entries()) {
      safeSend(m.ws, { t: "room-ended", by: user.username });
      try {
        m.ws.close(4002, "Room ended");
      } catch {}
    }
...
// TURN/STUN (TURN делает соединение стабильным между разными сетями)
const TURN_CFG = ${JSON.stringify(turnCfg)};
const ICE_SERVERS = [
  { urls: ["stun:stun.l.google.com:19302", "stun:stun1.l.google.com:19302"] },
];
if (TURN_CFG && TURN_CFG.urls && TURN_CFG.username && TURN_CFG.credential) {
  ICE_SERVERS.push(TURN_CFG);
}
...
async function startMedia(){
  try{
    ...
    alert("Не удалось получить доступ к камере/микрофону. Разреши доступ в браузере и обнови страницу.");
    throw e;
  }
...
function connectWS(){
  ...
  ws.onmessage = async (ev)=>{
    ...
    if(msg.t==="room-flags"){ renderFlags(msg.flags); return; }
...
function ensurePeer(peerId,label){
  ...
  // статус + автопочинка если зависло на checking
  pc.oniceconnectionstatechange = () => {
    const st = document.getElementById("st_"+peerId);
    if (st) st.textContent = String(pc.iceConnectionState || "").toUpperCase();
  ...
  pc.onconnectionstatechange = () => {
    const st = document.getElementById("st_"+peerId);
    if (st) st.textContent = String(pc.connectionState || "").toUpperCase();
  };
...
function attachRemote(peerId,label,stream){
  ...
  // autoplay fix: если браузер блокирует звук — включаем muted и пробуем снова
  v.play().catch(()=>{
    v.muted = true;
    v.play().catch(()=>{});
  });
  ...
  st.className="status"; st.id="st_"+peerId; st.textContent="CONNECTING";
...
server.listen(PORT, () => {
  ...
  if (TURN_URLS) console.log(`TURN_URLS: ${TURN_URLS}`);
